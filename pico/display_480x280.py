...
For Waveshare 3.7" displays (B&W only)

Auto-detects B&W based on data size!
"""
...
DISPLAY_WIDTH = 480
DISPLAY_HEIGHT = 280
EXPECTED_BYTES_BW = (DISPLAY_WIDTH * DISPLAY_HEIGHT) // 8    # 16,800 bytes
...
def epd_display_image(data):
    if len(data) != EXPECTED_BYTES_BW:
        print(f"✗ Invalid size: {len(data)} (expected {EXPECTED_BYTES_BW})")
        return
    
    print(f"Displaying {len(data)} bytes...")
