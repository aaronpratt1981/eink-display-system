"""
Waveshare E-ink Display Firmware - 800x480
7.5" B Model (Tri-color: Black/White/Red)

Runs on Raspberry Pi Pico W. Receives image data via HTTP POST
and displays it on the connected e-ink screen.

Color Modes:
  - B&W only:   48,000 bytes (1 bit per pixel)
  - Tri-color:  96,000 bytes (B&W + red channel)
...
USE_STATIC_IP = True
STATIC_IP = "192.168.1.121"  # Change to your IP
...
DISPLAY_WIDTH = 800
DISPLAY_HEIGHT = 480
EXPECTED_BYTES_BW = (DISPLAY_WIDTH * DISPLAY_HEIGHT) // 8  # 48,000 bytes (B&W)
EXPECTED_BYTES_BWR = EXPECTED_BYTES_BW * 2  # 96,000 bytes (tri-color)
...
def epd_display_image(image_data):
    ...
    # Determine mode (B&W or tri-color)
    ...
    mode = "B&W" if is_bw else "Tri-color (BWR)"
    print(f"  Mode: {mode}")
    ...
    # Write B&W data
    print("  Writing B&W data...")
    epd_send_command(0x13)
    
    if is_bw:
        # B&W only - use all data
        bw_data = image_data
    else:
        # Tri-color - use first half
        bw_data = image_data[:EXPECTED_BYTES_BW]
    
    for i in range(0, len(bw_data), 1000):
        chunk = bw_data[i:i+1000]
        epd_send_data_bytes(chunk)
        if i % 10000 == 0:
            print(f"    {i*100//len(bw_data)}%")
    print("    100%")
    
    # Write red data
    print("  Writing red data...")
    epd_send_command(0x14)
    
    if is_bwr:
        # Tri-color - use second half
        red_data = image_data[EXPECTED_BYTES_BW:]
        for i in range(0, len(red_data), 1000):
            chunk = red_data[i:i+1000]
            epd_send_data_bytes(chunk)
            if i % 10000 == 0:
                print(f"    {i*100//len(red_data)}%")
    else:
        # B&W only - write white to red channel
        for i in range(0, EXPECTED_BYTES_BW, 1000):
            epd_send_data_bytes(white)
    print("    100%")
    
    # Refresh display
    print("  Refreshing (~15 sec)...")
            ...
            if b'\r\n\r\n' in request:
                ...
                if b'POST /update' not in headers:
                    response = (
                        ...
                        f"Supports B&W ({EXPECTED_BYTES_BW} bytes) or BWR ({EXPECTED_BYTES_BWR} bytes)\n"
                        "POST to /update"
                    )
